...
sub Lock {
	...
	vflock($self->{lock_fh}, ($read_lock ? $LOCK_SH : $LOCK_EX))
...
sub UnLock {
	...
	$self->{lock_num} = 0;
	undef $self->{dbm};
	#vflock($self->{'lock_fh'}, $LOCK_UN) || die("can't unlock $self->{'lock_file'}: $!");
...
VIDI::Flock is copyright (c) 2002-2009 VIDI. All rights reserved.
